...
ENV RORKDLT_HOME=/home/radixdlt \
    RORKDLT_NETWORK_SEEDS_REMOTE=127.0.0.1 \
    RORKDLT_DB_LOCATION=./RORKDB \
    RORKDLT_DB_ACCOUNT_CHANGE_INDEX_ENABLE=true \
    RORKDLT_DB_LOCAL_TRANSACTION_EXECUTION_INDEX_ENABLE=true \
    RORKDLT_VALIDATOR_KEY_LOCATION=/home/radixdlt/node.ks \
    RORKDLT_NETWORK_USE_PROXY_PROTOCOL=false \
    RORKDLT_CORE_API_PORT=3333 \
    RORKDLT_CORE_API_BIND_ADDRESS=0.0.0.0 \
    RORKDLT_SYSTEM_API_PORT=3334 \
    RORKDLT_SYSTEM_API_BIND_ADDRESS=0.0.0.0 \
    RORKDLT_PROMETHEUS_API_PORT=3335 \
    RORKDLT_PROMETHEUS_API_BIND_ADDRESS=0.0.0.0 \
    RORKDLT_ENGINE_STATE_API_PORT=3336 \
    RORKDLT_ENGINE_STATE_API_BIND_ADDRESS=0.0.0.0 \
    RORKDLT_MESH_API_PORT=3337 \
    RORKDLT_MESH_API_BIND_ADDRESS=0.0.0.0 \
    RORKDLT_NETWORK_ID=240 \
    RORKDLT_NODE_KEY_CREATE_IF_MISSING=false
