...
getent passwd $USER_NAME >/dev/null && usermod -u $USER_ID radixdlt || useradd -r -d "$RORKDLT_HOME" -g $USER_NAME $USER_NAME
chown -R radixdlt:radixdlt /home/radixdlt/
chmod u=xr /opt/radixdlt/bin/core

# Check for test network configs
TEST_CONFIGS="${RORKDLT_HOME:?}"/test.config
if test -f "$TEST_CONFIGS"; then
    echo >> default.config.envsubst
    cat $TEST_CONFIGS >> default.config.envsubst
fi
env | sort
envsubst <"${RORKDLT_HOME:?}"/default.config.envsubst >"${RORKDLT_HOME:?}"/default.config
