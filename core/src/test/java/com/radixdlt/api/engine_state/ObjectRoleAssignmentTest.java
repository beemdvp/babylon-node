...
public final class ObjectRoleAssignmentTest extends DeterministicEngineStateApiTestBase {
  ...
  public void engine_state_api_returns_object_role_assignment() throws Exception {
    try (var test = buildRunningServerTest(defaultConfig())) {
      ...
      final var response =
          getAttachedModulesApi()
              .objectAttachedModulesRoleAssignmentPost(
                  new ObjectRoleAssignmentRequest().entityAddress(wellKnownAddresses.getXrd()));

      // Assert some owner role entry: (too complex for exact comparison):
      ...
      assertThat(
              response.getMainModuleRoles().stream()
                  .map(entry -> entry.getAssignment().getType())
                  .collect(Collectors.toSet()))
      ...
      final var roleAssignmentModule =
          response.getAttachedModules().stream()
              .filter(module -> module.getAttachedModuleId() == AttachedModuleId.ROLEASSIGNMENT)
              .findFirst()
              .orElseThrow();
      assertThat(roleAssignmentModule.getRoles()).isEmpty();

      // The metadata module fallbacks all its expected roles to Owner rule:
      final var metadataModule =
          response.getAttachedModules().stream()
              .filter(module -> module.getAttachedModuleId() == AttachedModuleId.METADATA)
              .findFirst()
              .orElseThrow();
      assertThat(
              metadataModule.getRoles().stream()
                  .map(RoleAssignmentEntry::getKey)
                  .collect(Collectors.toSet()))
          .isEqualTo(
              Set.of(
      ...
      assertThat(
              metadataModule.getRoles().stream()
                  .map(entry -> entry.getAssignment().getType())
                  .collect(Collectors.toSet()))
          .isEqualTo(Set.of(AssignmentType.OWNER));
    }
  }
}
