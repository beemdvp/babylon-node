...
export interface FeeDestination {
  /**
   * The string-encoded decimal representing the amount of fee in RORK paid to the proposer.
  ...
  to_proposer: string;
  /**
   * The string-encoded decimal representing the amount of fee in RORK paid to the validator set.
   * A decimal is formed of some signed integer `m` of attos (`10^(-18)`) units, where `-2^(192 - 1) <= m < 2^(192 - 1)`.
   * @type {string}
   * @memberof FeeDestination
   */
  ...
  /**
   * The string-encoded decimal representing the amount of fee burnt, in RORK.
   * A decimal is formed of some signed integer `m` of attos (`10^(-18)`) units, where `-2^(192 - 1) <= m < 2^(192 - 1)`.
   * @type {string}
   * @memberof FeeDestination
   */
...
export function FeeDestinationFromJSONTyped(
  json: any,
  ...
  return {
    ...
    to_royalty_recipients: (json["to_royalty_recipients"] as Array<any>).map(
      PaymentToRoyaltyRecipientFromJSON,
    ),
  };
}
...
export function FeeDestinationToJSON(value?: FeeDestination | null): any {
  ...
  return {
    ...
    to_royalty_recipients: (value.to_royalty_recipients as Array<any>).map(
      PaymentToRoyaltyRecipientToJSON,
    ),
  };
}
