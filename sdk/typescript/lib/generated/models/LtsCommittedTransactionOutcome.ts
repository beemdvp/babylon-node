...
export interface LtsCommittedTransactionOutcome {
  ...
  /**
   * The string-encoded decimal representing the total amount of RORK paid as fee (execution, validator tip and royalties).
 ...
 */
export function instanceOfLtsCommittedTransactionOutcome(
  value: object,
): boolean {
  let isInstance = true;
...
export function LtsCommittedTransactionOutcomeFromJSON(
  json: any,
): LtsCommittedTransactionOutcome {
  return LtsCommittedTransactionOutcomeFromJSONTyped(json, false);
}

export function LtsCommittedTransactionOutcomeFromJSONTyped(
  json: any,
  ...
  return {
    ...
    user_transaction_identifiers: !exists(json, "user_transaction_identifiers")
      ? undefined
      : TransactionIdentifiersFromJSON(json["user_transaction_identifiers"]),
    status: LtsCommittedTransactionStatusFromJSON(json["status"]),
    fungible_entity_balance_changes: (
      json["fungible_entity_balance_changes"] as Array<any>
    ).map(LtsEntityFungibleBalanceChangesFromJSON),
    non_fungible_entity_balance_changes: (
      json["non_fungible_entity_balance_changes"] as Array<any>
    ).map(LtsEntityNonFungibleBalanceChangesFromJSON),
    resultant_account_fungible_balances: (
      json["resultant_account_fungible_balances"] as Array<any>
    ).map(LtsResultantAccountFungibleBalancesFromJSON),
    total_fee: json["total_fee"],
  };
}

export function LtsCommittedTransactionOutcomeToJSON(
  value?: LtsCommittedTransactionOutcome | null,
): any {
  if (value === undefined) {
    return undefined;
  }
  ...
  return {
    ...
    user_transaction_identifiers: TransactionIdentifiersToJSON(
      value.user_transaction_identifiers,
    ),
    status: LtsCommittedTransactionStatusToJSON(value.status),
    fungible_entity_balance_changes: (
      value.fungible_entity_balance_changes as Array<any>
    ).map(LtsEntityFungibleBalanceChangesToJSON),
    non_fungible_entity_balance_changes: (
      value.non_fungible_entity_balance_changes as Array<any>
    ).map(LtsEntityNonFungibleBalanceChangesToJSON),
    resultant_account_fungible_balances: (
      value.resultant_account_fungible_balances as Array<any>
    ).map(LtsResultantAccountFungibleBalancesToJSON),
